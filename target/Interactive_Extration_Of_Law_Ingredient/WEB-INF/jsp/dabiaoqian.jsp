...
<html>
	...
	window.para=new Array();
 <c:forEach items="${list}" var="word" varStatus="status"><c:choose> <c:when test="${word=='/p'}">window.para.push(${status.count});</c:when></c:choose></c:forEach>
	window.curid=0;
	window.curcond=0;
...
&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;
		<div><span style="font-size:40px;font-weight:bold;">基本信息</span></div>
		&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;
		  <div> &nbsp;</div>
          ...
          <c:choose>
           <c:when test="${word=='/p'}">
              <div> &nbsp;</div>
           <div> <span style="font-size:40px;font-weight:bold;" id = "${status.count}title"></span></div>
          <div> <span  id="${status.count}"></span></div>
           <div> &nbsp;</div>
           <div>&nbsp; </div>
...
<!--选择模型类型-->
	<script type="text/javascript">
	var html = document.getElementById(para[0]+"title").innerHTML;
	document.getElementById(para[0]+"title").innerHTML = html+"案件描述";
	var html1 = document.getElementById(para[1]+"title").innerHTML;
	document.getElementById(para[1]+"title").innerHTML = html1+"审理查明";
	var html2 = document.getElementById(para[2]+"title").innerHTML;
	document.getElementById(para[2]+"title").innerHTML = html2+"判决结果";
	</script>
</body>
